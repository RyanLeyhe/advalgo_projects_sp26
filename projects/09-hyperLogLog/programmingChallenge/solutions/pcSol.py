...
class SlidingWindowHLL:
    """
    Sliding-window extension of HyperLogLog.

    The stream is divided into fixed-size blocks, each with its own HLL sketch.
    Old blocks are removed as the window slides.
    """
    ...
    def __init__(self, window_size, block_size, p):
        ...
        # Store completed blocks in order
        self.blocks = deque()

        # Current (incomplete) block
        self.current_block = HyperLogLog(p)
        self.current_block_count = 0

    def add(self, value):
        """
        Add a new element to the current block.
        When a block fills up, start a new one.
        """
        self.current_block.add_element(value)
        self.current_block_count += 1

        # Finalize block when it reaches block_size
    ...
    def query(self):
        """
        Estimate the number of distinct elements in the window
        by merging sketches from all active blocks.
        """
        ...
        # Include the current (partial) block
...
def main():
    """
    Read input, process stream operations, and answer queries.
    """
    lines = sys.stdin.read().strip().splitlines()
    n, W = map(int, lines[0].split())

    # accuracy parameter selected from typical values for HyperLogLog (2^P registers)
    P = 10

    # Block size trades accuracy for efficiency, selected somewhat arbitrarily
